...
// void set_learn_ctx(struct learn_ctx *lrn, 
...
void init_group_table(void) 
{
    ...
    table->nodes[0].buckets[0].actions[2].value = 0x0001; // pcp + vid
    ...
    table->nodes[0].buckets[0].actions[4].value = 0x0001; // pcp + vid
    ...
    table->nodes[1].buckets[0].actions[4].value = 0x0002; // pcp + vid
    ...
    table->nodes[1].buckets[0].actions[6].value = 0x0002; // pcp + vid
               ...
               struct output_frame_context *ctrl_frame)
{
    ...
    if (pkt->port == 4 || pkt->port == 5) {
        lrn.actions_dst[0].type = ACTION_PUSH_VLAN;
        lrn.actions_dst[0].value = 0x0001; // pcp + vid
    } else {
        lrn.actions_dst[0].type = ACTION_POP_VLAN;
        lrn.actions_dst[0].value = 0;
    }
    lrn.actions_dst[1].type = ACTION_OUTPUT;
    lrn.actions_dst[1].value = pkt->port;
...
{
    static uint8_t frame1[] = {
            0x00, 0xf0, 0x00, 0x00, 0x01, 0x01,
            0xf1, 0xf1, 0xf1, 0xff, 0xff, 0xff,
            0x08, 0x00, // end of ethernet header
            0x45, 0xc0, 0x00, 0x3c, 0xbd, 0x54,
            0x40, 0x00, 0x40, 0x06, 0x7e, 0xa5,
            0x7f, 0x00, 0x00, 0x01,
    ...
    static uint8_t frame2_tagged[] = {
            0xf1, 0xf1, 0xf1, 0xff, 0xff, 0xff,
            0x11, 0xff, 0xf2, 0xf3, 0xff, 0xff,
            0x81, 0x00, 0x00, 0x01, // vlan header
            0x08, 0x00, // end of ethernet header
            0x45, 0xc0, 0x00, 0x3c, 0xbd, 0x54,
            0x40, 0x00, 0x40, 0x06, 0x7e, 0xa5,
            0x7f, 0x00, 0x00, 0x01, // src_ip
    ...
    reset_output_frame_buffers();
    mac_load_frame(5, frame1, sizeof(frame1));
    if (receive(5, &prs_ctx, &io, &sfn))
        return 1;
    process_model(5, &prs_ctx, &sfn);
    transmit(&io);
    ...
    if (out_frames[6].frame.size != sizeof(frame1) ||
        memcmp(frame1, out_frames[6].frame.buf, sizeof(frame1)) != 0) {
        return 1;
    }   

    if (out_frames[2].frame.size != sizeof(frame1) ||
        memcmp(frame1, out_frames[2].frame.buf, sizeof(frame1)) != 0) {
        return 1;
    }
    ...
    if (out_frames[5].frame.size != sizeof(frame2_tagged) ||
        memcmp(frame2_tagged, out_frames[5].frame.buf,
               sizeof(frame2_tagged)) != 0) {
        return 1;
    }
...
int test_set_vlan(struct output_frame_context *out_frames,
                  struct output_frame_context *ctrl_frame)
{
    static uint8_t frame1_tagged[] = {
            0x00, 0x11, 0x21, 0xff, 0xff, 0xff,
            0x11, 0xff, 0xf2, 0xf3, 0xff, 0xff,
            0x81, 0x00, 0x00, 0x01, // vlan header
            0x08, 0x00, // end of ethernet header
            0x45, 0xc0, 0x00, 0x3c, 0xbd, 0x54,
    ...
    struct learn_ctx lrn;
    struct ethaddr mac = {
        .octets = {0x00, 0x11, 0x21, 0xff, 0xff, 0xff}
    };
    // struct ctrl_lrn_pkt *pkt = (void *) ctrl_frame->frame.buf;
    memcpy(lrn.src_mac.octets, &mac, sizeof(lrn.src_mac.octets));

    lrn.vlan_tag = 1;
    lrn.actions_src[0].type = ACTION_OUTPUT;
    lrn.actions_src[0].value = 5;
    lrn.actions_src[1].type = INSTR_GOTO_TABLE;
    lrn.actions_src[1].value = 1;
    lrn.act_len_src = 2;

    lrn.actions_dst[0].type = ACTION_SET_VLAN;
    lrn.actions_dst[0].value = 0x0002; // pcp + vid

    lrn.actions_dst[1].type = ACTION_OUTPUT;
    lrn.actions_dst[1].value = 5;
    ...
    reset_output_frame_buffers();
    mac_load_frame(0, frame1_tagged, sizeof(frame1_tagged));
    if (receive(0, &prs_ctx, &io, &sfn))
        return 1;
    process_model(0, &prs_ctx, &sfn);
    transmit(&io);
