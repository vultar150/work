...
void replicator(struct stage_fn *sfn, struct replicator_context *in_ctx)
{
    struct frame_buffer *fb_ptr = sfn->mmap_fb(in_ctx->location.fb_id);
    struct frame_buffer frame;
    struct frame_buffer out_frame;
    sfn->read_fb((uint8_t *)fb_ptr, (uint8_t *)&frame.data, in_ctx->location.framesz);
